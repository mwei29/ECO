...
variables <- c("perceived_stress_sum","CES_D_sum", "dweck_sum","sd_cogflu_uc","stress_pm", "ER_yes_pm","distr_pm", "min_pm","reap_pm","ER_yes_prompt_pm","distr_prompt_pm","min_prompt_pm","reap_prompt_pm")
for (vid in 1:length(variables)) { #vid<-3
  hist(data_b[[variables[vid]]], breaks = 20, border = "black",main = variables[vid], xlab = variables[vid], ylab = "# of Prompts")
}
...
```{r PSS ER, echo=TRUE,eval=TRUE,warning=F, message=F}
...
```{r PSS ER mod, fig.width=10, fig.height=8, echo=TRUE,eval=TRUE,warning=F, message=F}
# stress predicting whether or not ER strategy was selected
# CESD: sd=12.04
ER <- lm(ER_yes_pm ~ PSS_gmc*CESD_gmc+IBES_gmc+NIHCB_gmc+Group, data=data_b)
summary(ER)
...
```{r PSS ER mod strategy, fig.width=10, fig.height=8, echo=TRUE,eval=TRUE,warning=F, message=F}
# stress predicting ER strategy preference
# CESD
#distraction
distr <- lm(distr_pm~ PSS_gmc*CESD_gmc+IBES_gmc+NIHCB_gmc+Group, data=data_b)
...
```{r ESMstress ER, echo=FALSE,eval=FALSE,warning=F, message=F}
# stress predicting whether or not ER strategy was selected
...
```{r ESMstress ER mod, fig.width=10, fig.height=8, echo=TRUE,eval=TRUE,warning=F, message=F}
...
```{r ESMstress ER mod strategy, fig.width=10, fig.height=8, echo=TRUE,eval=TRUE,warning=F, message=F}
# stress predicting ER strategy preference
# CESD
#distraction
distr <- lm(distr_pm~ stress_pm_gmc*CESD_gmc+IBES_gmc+NIHCB_gmc+Group, data=data_b)
...
## Multilevel Modeling - hurdle model
Given the prevalence of 0 observation for specific strategy use & overall ER use, fit a hurdle model for multilevel modeling

### Stress predicting overall ER use & each strategy use
```{r withinER,echo=TRUE,eval=TRUE,warning=F, message=F}
hurdle_model <- hurdle(ER_yes ~ stress_w + stress_b + PSS_gmc + CESD_gmc + IBES_gmc + NIHCB_gmc + Group, 
                       data = data,
                       dist = "poisson")  # Assuming Poisson distribution for the count data
...
```{r withinER mod,echo=TRUE,eval=TRUE,warning=F, message=F}


```

### Moderation analysis: specific ER use
```{r withinER strategy mod,echo=TRUE,eval=TRUE,warning=F, message=F}


```

```{r hurdle pss, echo=FALSE,eval=FALSE,warning=F, message=F}
# stress predicting whether or not overall ER was used
# stress predicting ER strategy preference
hurdle_model <- hurdle(count_var ~ predictor | predictor, 
                       data = data,
                       dist = "poisson")  # Assuming Poisson distribution for the count data
