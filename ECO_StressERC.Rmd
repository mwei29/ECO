...
```{r QC, eval=TRUE, echo=FALSE, warning=FALSE, cache=TRUE, message=FALSE}
...
ESM_test$reap_yes <- ifelse(is.na(ESM_test$ER_yes) | (is.na(ESM_test$esm_howinfluence) & is.na(ESM_test$er_othermethods_posreapp)), 
                            ...
                            1, 0))
ESM_clean <- ESM_test[, c("PID", "Group", "completed", "esm_stressed",  "ER_yes", "distr_yes", "min_yes", "reap_yes", "sd.cogflu.uc", "perceived_stress_sum", "CES_D_sum", "dweck.sum", "Gender", "Age", "Race2_1", "Race2_2", "Race2_3", "Race2_4", "Race2_5", "Race2_6", "Race2_7")]
ESM_clean <-ESM_clean %>%
  group_by(PID) %>%
  mutate (Group = factor(Group, levels = 0:2, labels = c("YA","CN","MCI")),
          stress_mean = mean(esm_stressed,na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(PSS_gmc = scale(perceived_stress_sum, center = T, scale = F),
         ...
         stress_gmc = scale(stress_mean, center = T, scale = F)) %>%
  as.data.frame()
          
#between person
data_b <- ESM_clean%>%
  group_by(PID, Group, sd.cogflu.uc, perceived_stress_sum, CES_D_sum,dweck.sum,stress_mean,PSS_gmc,CESD_gmc,IBES_gmc,NIHCB_gmc,stress_gmc) %>%
  summarize(n_total_completed = sum(completed==1,na.rm = TRUE), 
            n_total_ER = sum(ER_yes,na.rm = TRUE),
            perc_ER = n_total_ER/n_total_completed,
            n_distr = sum(distr_yes, na.rm = TRUE),
            n_min = sum(min_yes, na.rm = TRUE), 
            n_reap = sum(reap_yes, na.rm = TRUE),
            perc_distr =ifelse(n_total_ER > 0, n_distr / n_total_ER, 0),
            perc_min = ifelse(n_total_ER > 0, n_min / n_total_ER, 0),
            perc_reap = ifelse(n_total_ER > 0, n_reap / n_total_ER, 0)) %>%
  ungroup() %>%
  as.data.frame()
view(data_b)
```
...
for (vid in 1:length(variables)) { #vid<-1
  hist(data_b[[variables[vid]]], breaks = 20, border = "black",main = variables[vid], xlab = variables[vid], ylab = "# of Participants")
}
par(mfrow = c(1, 1))
# boxplot to check the relationship between cognitive fluid score and cognitive impairment
boxplot(data_b$sd.cogflu.uc~data_b$Group, main="cognitive fluid score by cognitive impairment group", xlab= "Group", ylab="Score", 
        col = c("lightblue", "lightgreen", "lightcoral"),
        border = "black",
        notch = TRUE,
        varwidth = TRUE)
#OVerall stats including centered variables
describe(data_b[variables])
describe(ESM_clean[c("completed", "esm_stressed","ER_yes","distr_yes","min_yes","reap_yes")])
```
...
library(lmerTest)
library(performance)
fit_logit <- glmer(ER_yes ~ 1 + (1 | PID), data = ESM_clean, family = binomial(link = "logit"))
model_performance(fit_logit) 
#ICC = .463
fit_logit <- glmer(distr_yes ~ 1 + (1 | PID), data = ESM_clean, family = binomial(link = "logit"))
model_performance(fit_logit) 
#ICC = 0.464
fit_logit <- glmer(min_yes ~ 1 + (1 | PID), data = ESM_clean, family = binomial(link = "logit"))
model_performance(fit_logit) 
#ICC = 0.435
fit_logit <- glmer(reap_yes ~ 1 + (1 | PID), data = ESM_clean, family = binomial(link = "logit"))
model_performance(fit_logit) 
#ICC = 0.48
```
...
correlation_results <- corr.test(data_b[c("perc_ER","perc_distr","perc_min","perc_reap","PSS_gmc","CESD_gmc","IBES_gmc")])
correlation_results
