...
fit0 <- glmer(esm_stressed ~ 1 + (1 | PID), data = data_exclude)
...
#I find this model to be very complicated
ER <- glmer(ER_yes ~ stress_lag_w*Group + stress_lag_b*Group + control_lag_w + control_lag_b + time0m +
              (1 + time0m | PID), 
            family = binomial(link = "logit"),
            data = data_exclude,
            control = glmerControl(optimizer = "bobyqa"))
...
ER <- glmer(ER_yes ~ stress_lag_w*Group + stress_lag_b*Group + time0m +
              (1 + time0m | PID), 
            family = binomial(link = "logit"),
            data = data_exclude,
            control = glmerControl(optimizer = "bobyqa"))
...
```{r mod graphs, echo=FALSE, eval=FALSE, fig.width=10, fig.height=8, warning=F, cache=FALSE, message=F}
