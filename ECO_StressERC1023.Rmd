...
data <-ESM_clean %>%
  ...
  mutate(
    ...
    ER_need_lag = lag (esm_wanted_change,1),
    ER_need_gmc_lag = lag (ER_need_gmc,1)
  ) %>%
  ungroup() %>%
  group_by(PID) %>%
...
variables <- c("completed_sum","ER_yes_sum","CES_D_sum", "stress_pm","NA_pm", "ER_need_pm", "ER_yes_pm","distr_pm", "min_pm","reap_pm")
...
data$Group <- relevel(data$Group, ref="CN")
ER <- glmer(ER_yes ~ stress_lag_w + stress_lag_b + ER_need_gmc_lag + Group + 
                        (1 + stress_lag_w | PID), 
...
control <- lmer(stress_w~control_w+(1+control_w|PID),data=data)
summary(control)
...
```{r NAER,echo=TRUE,eval=TRUE,warning=F, cache=FALSE, message=F}
ER <- glmer(ER_yes ~ NA_lag_w + NA_lag_b + ER_need_gmc_lag + Group + 
                        (1 + NA_lag_w | PID), 
                        family = binomial(link = "logit"),
                        data = data)
summary(ER)
